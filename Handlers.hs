            ...
            defaultLayout $ do 
               ...
               [whamlet|
                   <div ."container">
                    ...
                    <footer>
                        <p>MyStore Copyright &copy; 2015 <a href="#" title="">Terms &middot; <a href="#" title="">Privac
...
getListaContatoR = do 
                listaC <- runDB $ selectList [] [Asc ContatoNome]
                defaultLayout $ do 
                addStylesheet $ StaticR css_bootstrap_css
                addStylesheet $ StaticR css_style_css
                addScript $ StaticR js_bootstrap_js
                    ...
                    <div ."container">
                        ...
                        <table ."table table-striped table-bordered">
                            <thead>       
                                ...
                                <th>Mensagem
                            <tbody>
                                $forall Entity cid contato <- listaC
                                    <tr>
...
getListaPedidoR  = do
         listaP <- runDB $ selectList [PedidoStatus ==. 1] [Asc PedidoId]
         defaultLayout $ do 
         [whamlet|
             <h1> Pedidos cadastrados:
             $forall Entity pid pedido <- listaP
                    <h2>#{pedidoStatus pedido}
                    <p>
                        Quantidade do Pedido: #{pedidoQuantidade pedido}
         |] 
         toWidget [lucius|
            form  { display:inline; }
            input { background-color: #ecc; border:0;}
...
getListagemPedidoR :: Handler Html
getListagemPedidoR = do
    musuarioId <- maybeIdUsuario
    case musuarioId of
        Nothing ->  do
            setMessage "usuario não autenticado"
            redirect LoginR
        Just usuarioId -> do
            -- pedidos <- runDB $ rawSql "SELECT A.nome, A.preco, B.quantidade FROM Produto A INNER JOIN Pedido B on A.id = B.produtoid WHERE B.status = 0 and B.usuarioId = usuario.id"
            defaultLayout $ do 
...
getPedidoR pid = do
    ...
    musuarioId <- maybeIdUsuario -- Exemplo zica de como extrair o ID do usuário na sessão, é vida loka
    case musuarioId of
        Nothing -> do
            setMessage "usuario não autenticado"
            redirect LoginR
                                ...
                                <div ."row">
                                    <div ."col-md-8">
                                        ...
                                        <form action=@{PedidoCadastrarR} method=post >
                                            ...
                                            <input type=hidden name=usuarioid value=#{show $ fromSqlKey usuarioId}>
